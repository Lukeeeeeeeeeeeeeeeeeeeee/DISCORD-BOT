...
function getPerformanceCategory({ percentile, warnings, recruits7d, minReq, absent }) {
  if (absent) return { bucket: 'ABSENT', label: 'Absent', color: 0x808080 };
  ...
  if (p < 0.4) return { bucket: 'ATTENTION', label: 'Attention', color: 0xFFA500 };
...
async function execute(interaction, _client, dbHandle = null) {
  ...
  const recruiterIds = await resolveRecruiterIdsForTeam({ guild: interaction.guild, team, db, guildId });
  ...
  if (placeholders) {
    const [absences, warnings, weekCalcs, points] = await Promise.all([
      db.all(
        `SELECT recruiter_id, start_date, end_date FROM absences WHERE guild_id = ? AND active = 1 AND end_date >= date("now") AND recruiter_id IN (${placeholders})`,
        guildId,
        ...recruiterIds
      ).catch(() => []),
      db.all(
      ...
      db.all(
        `SELECT id, points FROM recruiters WHERE guild_id = ? AND id IN (${placeholders})`,
        guildId,
        ...recruiterIds
      ).catch(() => [])
    ]);
  ...
  const avgWeeklyMap = await getAverageWeeklyRecruitsMap(db, recruiterIds, guildId).catch(() => new Map());

  const rawResults = await runWithConcurrency(recruiterIds, REPORT_CONCURRENCY, async (id) => {
    const member = recruiterMemberMap.get(id)
      || (interaction.guild.members && interaction.guild.members.cache
    ...
    if (weekCalc && Number.isFinite(Number(weekCalc.recruits7d))) {
      ...
      stats7d = await calculate7DayStats(db, id, interaction.guild, { ...statsWindow, guildId })
        .catch(() => ({ recruits7d: 0, activityRate: 0, verifyRate: 0, retention: 0 }));
    }
    const avgRecruitsWeek = avgWeeklyMap.has(id)
      ? Number(avgWeeklyMap.get(id) || 0)
      : await getAverageWeeklyRecruits(db, id, guildId).catch(() => 0);
    const absence = absencesMap.get(id) || null;
    const activeWarnings = warningsMap.get(id) || 0;
    const newStaffCheck = await isNewStaff(db, id, { guildId }).catch(() => false);
    ...
    if (minReq == null) {
      previousMinReq = await getPreviousMinReq(db, id, { guildId }).catch(() => null);
  ...
  const buckets = [
    ...
    { key: 'ATTENTION', title: 'ATTENTION', color: 0xFFA500 },
    ...
    { key: 'ABSENT', title: 'ABSENT', color: 0x808080 }
  ];
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x00AAFF)
    .setDescription('Performance score: recruits (40%), verify rate (25%), retention (20%), warnings penalty (15%). New recruiters are grouped separately; buckets are relative percentiles (warnings/absence override).')
    .setTimestamp();
  ...
  for (const b of buckets) {
    ...
    const lines = arr.map(x => {
      ...
      const warn = x.activeWarnings > 0 ? ` W${x.activeWarnings}` : '';
  ...
  embed.setFooter({
    text: `Total: ${results.length} | NEW${newly} | FAIL${failing} ATTN${attention} PASS${passing} SUCC${succeeding} ABS${absent}`
  });
