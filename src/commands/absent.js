...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!dateRegex.test(endDate)) {
      return interaction.reply({ content: 'Invalid date format. Use YYYY-MM-DD format.' });
    }

    // Parse and validate date
    const absenceDate = new Date(endDate);
    if (isNaN(absenceDate.getTime())) {
      return interaction.reply({ content: 'Invalid date. Please use a valid date in YYYY-MM-DD format.' });
    }
