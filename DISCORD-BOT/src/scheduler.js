...
function start(client, db) {
  ...
  trackScheduledJob(cron.schedule('0 0 1 * *', async () => {
    ...
    try {
      ...
      // AECS: Clean up old vault log files
      try {
        const removed = AECS.vault && typeof AECS.vault.cleanupOldFiles === 'function'
          ? AECS.vault.cleanupOldFiles(90)
          : 0;
        if (removed > 0) {
          console.log(`[scheduler] Pruned ${removed} old AECS vault file(s)`);
        }
      } catch (vaultErr) {
        console.error('Failed to prune old AECS vault files:', vaultErr);
      }
