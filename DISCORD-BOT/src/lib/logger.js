...
const { shouldSkipKey } = require('./aecs/sanitize');
...
function inferCodeFromScope(scope, error) {
  ...
  if (value.startsWith('service.recruit') || value.startsWith('service.invite') || value.startsWith('recruiting') || value.startsWith('service.revoke') || value.startsWith('service.recruiter')) return 'RECRUIT-500';
  if (value.startsWith('economy')) return 'ECONOMY-500';
  if (value.startsWith('dm.')) return 'DM-500';
  if (value.startsWith('analytics')) return 'ANALYTICS-500';
  return 'SYS-500';
}
