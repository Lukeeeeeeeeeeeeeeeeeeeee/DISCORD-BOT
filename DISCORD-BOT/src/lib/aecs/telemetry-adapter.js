...
class TelemetryAdapter {
  ...
  buildPayload(record) {
    ...
    const stackPreview = truncateText(sanitizeString(record.stack || ''), 950);
   ...
   * @returns {Promise<{ok: boolean, retryable: boolean}>} Result of the send attempt
   */
  async sendToWebhook(url, payload) {
    if (!url || typeof this.fetchImpl !== 'function') return { ok: false, retryable: false };
    ...
    try {
      ...
      if (!response || response.ok === false) {
        ...
        return { ok: false, retryable: status >= 500 };
      }
  ...
  /**
   * Send payload to a webhook with automatic retry on transient failures.
   * @param {string} url - Webhook URL to send to
   * @param {object} payload - JSON-serializable payload
   * @returns {boolean} true if the webhook accepted the payload after all retries
   */
  async sendToWebhookWithRetry(url, payload) {
  ...
  /**
   * Send a telemetry record to configured webhooks.
   * @param {object} record - Telemetry record to send
  ...
  async send(record) {
    ...
    if (severity === 'FATAL') {
      const results = await Promise.all(targets.map(url => this.sendToWebhookWithRetry(url, payload)));
    ...
    for (const url of targets) {
      const ok = await this.sendToWebhookWithRetry(url, payload);
      if (ok) {
