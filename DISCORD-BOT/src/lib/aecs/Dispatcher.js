...
function normalizeMessage(message) {
  ...
  return String(message)
    ...
    .replace(/\b\d+\b/g, '[NUM]') // whole numbers only (after UUIDs)
    .trim();
}
...
class Dispatcher {
  constructor(options = {}) {
    ...
    this.suppressionMapMaxSize = Number.parseInt(options.suppressionMapMaxSize || '10000', 10);
  ...
  computeImpact(definition, sanitizedMeta, traceContext) {
    ...
    if (definition && typeof definition.escalator === 'function') {
      try {
        ...
        console.error('[AECS] Escalator failed:', error && error.stack ? error.stack : error);
      }
    }
    return impact;
  }
  ...
  shouldSuppress(fingerprint, code, scope, severity) {
    ...
    if (!state) {
      ...
      // FIND-01: Evict oldest entries if map exceeds max size
      if (this.suppressionMap.size > this.suppressionMapMaxSize) {
        const entries = [...this.suppressionMap.entries()]
          .filter(([, s]) => s.suppressed <= 0)
          .sort((a, b) => a[1].windowStart - b[1].windowStart);
        const evictCount = Math.min(
          Math.ceil(this.suppressionMapMaxSize * 0.2),
          entries.length
        );
  ...
  async flushSuppressionSummaries() {
    ...
    for (const [fingerprint, state] of this.suppressionMap.entries()) {
      ...
      const summaryRecord = {
        ...
        hash: createFingerprint('aecs.circuit_breaker', state.code, state.scope),
        hashId: hashIdFromFingerprint(createFingerprint('aecs.circuit_breaker', state.code, state.scope))
      };
  ...
  async dispatch(error, options = {}) {
    ...
    const record = {
      ...
      severity,
      impact,
      domain: dictionaries.getDomainForCode(codexError.code),
      scope,
      message,
