...
function getRecruitJoinLimitMinutes() {
  const configured = Number.parseInt(process.env.RECRUIT_MAX_JOIN_MINUTES, 10);
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!team) {
        const { RECRUITER_ROLE_IDS: configuredRoles } = require('../../constants');
        const memberRoleIds = recruiterMemberForTeam && recruiterMemberForTeam.roles && recruiterMemberForTeam.roles.cache && typeof recruiterMemberForTeam.roles.cache.keys === 'function'
          ? Array.from(recruiterMemberForTeam.roles.cache.keys())
          : [];
        void logRuntimeEvent('warn', 'command.recruit.noTeamRole', 'Recruiter lacks team-specific recruiter role', {
          guildId,
          recruiterId: recruiterMemberForTeam ? recruiterMemberForTeam.id : null,
          memberRoleIds: JSON.stringify(memberRoleIds),
          hasGenericRecruiter: recruiterMemberForTeam && recruiterMemberForTeam.roles && recruiterMemberForTeam.roles.cache && typeof recruiterMemberForTeam.roles.cache.has === 'function'
            ? recruiterMemberForTeam.roles.cache.has(require('../../constants').ROLE_IDS.RECRUITER)
            : false,
          configuredTeamRoles: JSON.stringify(configuredRoles),
          regionTag: regionTag || null,
          isAdmin: !!hasAdminOrStaffPermissions(recruiterMemberForTeam)
        });
      }
      ...
      if (!team) {
        ...
        try {
          ...
          reportRecruitError('command.recruit.resolveTeamError', resolveError, {
            guildId,
            recruiterId: recruiterMemberForTeam ? recruiterMemberForTeam.id : null
          });
        }
        if (!team) {
          ...
          reportRecruitError('command.recruit.teamResolveFailed', new Error('resolveRecruitTeam returned null'), {
            guildId,
            recruiterId: recruiterMemberForTeam ? recruiterMemberForTeam.id : null,
            configuredRecruiterRoleIds: JSON.stringify(RECRUITER_ROLE_IDS)
          });
        }
      }
      if (!team) {
      ...
      const maxJoinMinutes = getRecruitJoinLimitMinutes();
      if (minutesSinceJoin > maxJoinMinutes) {
        const maxHours = Math.round((maxJoinMinutes / 60) * 10) / 10;
        return replyError(interaction, `Cannot give roles to someone who joined more than ${maxHours} hour(s) ago.`);
      }
