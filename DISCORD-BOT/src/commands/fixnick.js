...
function isStatusToken(value) {
    ...
    return /^\d+(?:\.\d+)?\s*\/\s*2$/.test(normalized);
}
...
function cleanIgnSegment(value) {
    ...
    out = out.replace(/^\d+(?:\.\d+)?\s*\/\s*2\s*[|:-]?\s*/i, '').trim();
    out = out.replace(/\s*[|:-]?\s*\d+(?:\.\d+)?\s*\/\s*2$/i, '').trim();
    out = out.replace(/^(EU|NA|AS|ME|AF|SA)\s*[|:-]\s*/i, '').trim();
    out = out.replace(/\s*[|:-]\s*(EU|NA|AS|ME|AF|SA)$/i, '').trim();
    out = out.replace(/\s+(EU|NA|AS|ME|AF|SA)\s+\d+(?:\.\d+)?\s*\/\s*2$/i, '').trim();
    out = out.replace(/^\s*(EU|NA|AS|ME|AF|SA)\s+\d+(?:\.\d+)?\s*\/\s*2\s*/i, '').trim();
