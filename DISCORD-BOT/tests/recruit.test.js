...
describe('/recruit command', () => {
  ...
  test('rejects if joined more than 24 hours ago', async () => {
    const { interaction, guildMember } = makeInteraction();
    guildMember.joinedAt = new Date(Date.now() - (25 * 60 * 60 * 1000)); // 25 hours
    const cmd = require('../src/commands/recruiting/recruit.js');
    await cmd.execute(interaction);
    const replyArg = interaction.reply.mock.calls[0][0];
    const desc = replyArg.embeds ? replyArg.embeds[0].data.description : replyArg.content;
    expect(desc).toBe('Cannot give roles to someone who joined more than 24 hour(s) ago.');
  });

  test('respects RECRUIT_MAX_JOIN_MINUTES env var', async () => {
    const original = process.env.RECRUIT_MAX_JOIN_MINUTES;
    process.env.RECRUIT_MAX_JOIN_MINUTES = '360'; // 6 hours
    const { interaction, guildMember } = makeInteraction();
    guildMember.joinedAt = new Date(Date.now() - (7 * 60 * 60 * 1000)); // 7 hours
    const cmd = require('../src/commands/recruiting/recruit.js');
    await cmd.execute(interaction);
    const replyArg = interaction.reply.mock.calls[0][0];
    const desc = replyArg.embeds ? replyArg.embeds[0].data.description : replyArg.content;
    expect(desc).toBe('Cannot give roles to someone who joined more than 6 hour(s) ago.');
    process.env.RECRUIT_MAX_JOIN_MINUTES = original;
  });
